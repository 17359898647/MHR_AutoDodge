...
local function SaveSettings()
    json.dump_file("longsword.json", ConfigManager)
end

local function LoadSettings()
    local loadedSettings = json.load_file("longsword.json");
    if loadedSettings ~= nil
        and loadedSettings.kabutowariShellNum ~= nil
        and loadedSettings.loopAuraLevel ~= nil
        and loadedSettings.nadaoTime ~= nil
    then
...
sdk.hook(
    ...
    function(args)
        if initMaster() == true then
            ...
            if hitinfo:get_field("<AttackData>k__BackingField"):get_field('_UserData'):get_field(
                    '<Type>k__BackingField') == 3 and hitinfo:get_field("<AttackData>k__BackingField"):get_field('_DamageType') >= 0 then
    ...
    function(args)
        ...
        if damage_owner == "MasterPlayer" then
            if Isjianqie == true then
                if player:get_Character():call("changeActionRequest(app.AppActionDef.LAYER, ace.ACTION_ID, System.Boolean)", 0, actionID, true) then
                    damage_owner = ""
                    Isjianqie = false
                end
            end
